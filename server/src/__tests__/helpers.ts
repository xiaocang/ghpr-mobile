...
export async function initSchema(): Promise<void> {
  ...
  await env.DB.exec("CREATE TABLE IF NOT EXISTS runners (id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL UNIQUE, pairing_token_hash TEXT NOT NULL, label TEXT, user_id TEXT NOT NULL, github_login TEXT, last_poll_status TEXT, last_poll_error TEXT, last_poll_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), last_seen_at TEXT, notif_last_modified TEXT)");
  await env.DB.exec("CREATE INDEX IF NOT EXISTS idx_runners_pairing_token_hash ON runners (pairing_token_hash)");
  await env.DB.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_runners_user_id ON runners (user_id)");
